...
export default function AddVehicleModal({ isOpen, onClose, onSuccess }: AddVehicleModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (registrationNo) {
      const regRegex = /^[A-Z]{2}[ -]?[0-9]{1,2}(?:[ -]?[A-Z])?(?:[ -]?[A-Z]*)?[ -]?[0-9]{4}$/
      if (!regRegex.test(registrationNo)) {
        setError('Invalid registration number format. Example: MH 12 AB 1234, DL03CW3121')
        setLoading(false)
        return
      }
    }
      ...
      <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-foreground">Add New Vehicle</h2>
          </div>
          ...
          <form id="add-vehicle-form" onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-2 gap-5">
              ...
              <div className="space-y-2">
                ...
                <select required name="type" className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
            ...
            <div className="space-y-2">
              ...
              <Input name="registration_no" placeholder="e.g. MH 12 AB 1234" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <div className="space-y-2">
                ...
                <Input type="number" name="year" placeholder="2023" min="1990" max={new Date().getFullYear()} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-2">
                ...
                <Input required type="number" name="capacity_kg" placeholder="5000" min="0" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">Avg Cost/km (₹) *</label>
                <Input required type="number" step="0.1" name="avg_cost_per_km" placeholder="12.5" min="0" />
              </div>
            </div>
          </form>
        </div>
